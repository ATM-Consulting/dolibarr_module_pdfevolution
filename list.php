...
$PDOdb = new TPDOdb;
$object = new Tpdfevolution;

$hookmanager->initHooks(array('pdfevolutionlist'));
...
llxHeader('',$langs->trans('pdfevolutionList'),'','');

//$type = GETPOST('type');
//if (empty($user->rights->pdfevolution->all->read)) $type = 'mine';

// TODO ajouter les champs de son objet que l'on souhaite afficher
$sql = 'SELECT t.rowid, t.ref, t.label, t.date_cre, t.date_maj, \'\' AS action';

$sql.= ' FROM '.MAIN_DB_PREFIX.'pdfevolution t ';

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('pdfevolution', 1).')';
//if ($type == 'mine') $sql.= ' AND t.fk_user = '.$user->id;


$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_pdfevolution', 'GET');

$nbLine = !empty($user->conf->MAIN_SIZE_LISTE_LIMIT) ? $user->conf->MAIN_SIZE_LISTE_LIMIT : $conf->global->MAIN_SIZE_LISTE_LIMIT;

$r = new Listview($db, 'pdfevolution');
echo $r->render($sql, array(
	...
	,'search' => array(
		...
		,'status' => array('recherche' => TPdfevolution::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
		'rowid'
	...
	,'liste' => array(
		'titre' => $langs->trans('pdfevolutionList')
		,'image' => img_picto('','title_generic.png', '', 0)
		,'picto_precedent' => '<'
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('Nopdfevolution')
